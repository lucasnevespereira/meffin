...
/** The most recent occurrence of a series. It defines what the future looks like. */
...
export function projectSeries(
  ...
  for (const head of heads) {
    const start = Math.max(from, head.monthKey);
...
export function missingMonths(head: SeriesHead, through: MonthKey, occupied: Set<string>): MonthKey[] {
  ...
  for (let key = head.monthKey + 1; key <= last; key++) {
...
export function toSeriesHead(row: {
  ...
  return {
    ...
    monthKey: row.monthKey ?? monthKeyFromDateString(row.date),
